...
class Song(object):
    ...
    class Sinfo:
        ...
        def format_output(self, play_type):
            embed = discord.Embed(title=play_type, description=f"[{self.title}]({self.webpage_url})", color=config.EMBED_COLOR)
            ...
            if self.duration is not None:
                embed.add_field(name=config.SONGINFO_DURATION,
                                value=f"{datetime.timedelta(seconds=self.duration)}", inline=False)
            else:
                embed.add_field(name=config.SONGINFO_DURATION,
